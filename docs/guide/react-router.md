...
> [React Router](https://v5.reactrouter.com/web/guides/quick-start) - это библиотека для реализации маршрутизации на стороне клиента в `React-приложениях`.

[Примеры использования](https://v5.reactrouter.com/web/example/basic).

_Обратите внимание_: данное руководство посвящено `React Router` 5 версии, в 6 версии `API` рассматриваемой библиотеки претерпело несколько значительных изменений.
...
Если `Route` не имеет пропа `path` и поэтому всегда совпадает, мы получаем его ближайшего предка. То же самое справделиво для `withRouter`.
...
Маршрутизация на стороне сервера немного отличается ввиду отсутствия состояния. Основная идея состоит в том, что компонент верхнего уровня оборачивается в `<StaticRouter>` вместо `<BrowserRoute>`. Затем мы передаем роутеру запрашиваемый URL и контекст.
...
Делаем то же самое. Создаем компонент, добавляющий контекст, и рендерим его в приложении для получения разных статус-кодов.
...
## Восстановление прокрутки (scroll restoration)

### Прокрутка к началу страницы

В большинстве случаев нам требуется выполнить прокрутку к началу страницы из-за длинного контента, который при навигации, остается "прокрученным". Этого можно добиться с помощью компонента `<ScrollToTop>`, который будет выполнять прокрутку при каждой навигации:
