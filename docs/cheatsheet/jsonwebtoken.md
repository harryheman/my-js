...
> [jsonwebtoken](https://github.com/auth0/node-jsonwebtoken) - это библиотека для создания (генерации) и подтверждения токенов, используемых для аутентификации/авторизации пользователей. Данная библиолтека является реализацией [JSON Web Tokens](https://jwt.io/) для `Node.js`.
