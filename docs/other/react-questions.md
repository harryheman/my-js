...
<div>
  {/* Комментарий, состоящий из
    нескольких строк */}
  {`Добро пожаловать, ${user}. Давай изучать React`}
</div>
```
...
### Поддерживают ли "ленивые" (lazy) функции именованный экспорт?
...
### Что такое предохранители (Error Boundaries) в React 16?
...
### Для чего предназначен метод `render()` в `react-dom`?
...
При передаче опционального колбека, он будет вызван после рендеринга или обновления.
...
### Почему следует избегать использования индексов в качестве ключей?
...
### Правильно ли использовать `setState()` в методе `componentWillMount()`?
...
### Как реализовать рендеринг на стороне сервера или SSR?
...
### Что такое CRA и в чем заключаются преимущества его использования?
...
6. Скрипт для сборки `JS`, `CSS` и статических файлов для продакшна с хэшами в названиях и картами ресурсов (source maps).

### Назовите методы жизненного цикла, относящиеся к монтированию
...
### Как рекомендуется называть компоненты?
...
Оптимальным решением является определение места, где `setState()` может быть вызван после размонтирования компонента, и его удаление. Такие ситуации обычно возникают в колбеках, когда компонент ожидает получения некоторых данных и размонтируется до их получения. В идеале, все колбеки должны отключаться в `componentWillUnmount()` перед размонтированием.

### Какие события указателя (Pointer Events) поддерживаются в React?
...
### В чем разница между `super()` и `super(props)` в классовых компонентах React?
...
### Что такое массив React PropType с формой (shape)?
...
### Как реализовать условное применение классовых атрибутов?
...
### Как повторно отрендерить слой представления при изменении размеров браузера?
...
### Как следить за изменением состояния компонента?
...
### Как рекомендуется удалять элемент из массива в состоянии React?
...
### Как красиво отобразить JSON с помощью React?
...
Вы можете использовать проп `ref` через колбек в качестве ссылки на `HTMLInputElement`,  сохранить эту ссылку в свойстве класса и использовать ее для вызова события в обработчике с помощью метода `click()`.
...
### Можно ли использовать `async/await` в обычном React?
...
*React Motion* и *React Spring* являются самыми популярными библиотеками для работы с анимацией в экосистеме React.
...
### Что такое рендер-пропы (Render Props)?
...
### Для чего предназначены методы `push()` и `replace()` объекта `History`?
...
Существует, как минимум, 3 способа реализовать программный роутинг/навигацию в компоненте:
...
### Когда можно получить предупреждение "Router может содержать только один дочерний элемент"?
...
### Как передать параметры методу `history.push` в `React Router 4`?
...
### Как реализовать автоматическое перенаправление после выполнения входа в систему?
...
### Как использовать FormattedMessage в качестве заместителя в React Intl?
...
### Что такое поверхностный рендеринг (Shallow Rendering) в терминах тестирования?
